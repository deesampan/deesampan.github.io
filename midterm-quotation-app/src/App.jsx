...
function App() {
  ...
  const addItem = () => {
    ...
    if (dataItems.length>0&&check_dup(item)){
      return
    }
  ...
  function check_dup(item){
    let item_name = item.name
    let check = false
    dataItems.forEach(each => {
      let each_name = each.item
      let each_price = each.ppu
      


      if (each_name == item_name && Number(each_price) == Number(ppuRef.current.value)){
        each.qty = Number(each.qty) + Number(qtyRef.current.value)
        each.dis = disRef.current.value
        check = true
      }
    });
        ...
        <Col md={4} style={{ backgroundColor: "#e4e4e4" }}>
          ...
          <Row>
            <Col>
              ...
              <Form.Control type="number" ref={disRef} defaultValue={1} />
            </Col>
          </Row>
          <hr />
