...
function QuotationTable({ data, clearDataItems, deleteByIndex }) {
  ...
  const total_dis = data.reduce((acc, v) => acc + (v.qty * v.ppu - v.dis), 0);
      ...
      <Table striped bordered hover>
          ...
          data.map((v, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td className={style.textRight}>{v.dis}</td>
                <td className={style.textRight}>{amount - v.dis}</td>
              </tr>
            );
          })
        }</tbody>
        <tfoot>
          <tr>
            ...
            <td colSpan={2} className={style.textRight}>
              Total Discount
            </td>
            <td className={style.textRight}>{total_dis}</td>
          </tr>
        </tfoot>
      </Table>
    </Container>
